...
class SDL_Pi_HDC1000:
    def __init__(self, twi=1, addr=HDC1000_ADDRESS):
        ...
        self._bus.write_byte_data(
            HDC1000_ADDRESS, HDC1000_CONFIGURATION_REGISTER, config >> 8)
    ...
    def readManufacturerID(self):
        ...
        data = self._bus.read_i2c_block_data(
            HDC1000_ADDRESS, HDC1000_MANUFACTURERID_REGISTER, 2)
        return data[0] * 256 + data[1]

    def readDeviceID(self):

        data = self._bus.read_i2c_block_data(
            HDC1000_ADDRESS, HDC1000_DEVICEID_REGISTER, 2)
        return data[0] * 256 + data[1]
    ...
    def readSerialNumber(self):
        ...
        data = self._bus.read_i2c_block_data(
            HDC1000_ADDRESS, HDC1000_SERIALIDHIGH_REGISTER, 2)
        serialNumber = data[0]*256 + data[1]

        data = self._bus.read_i2c_block_data(
            HDC1000_ADDRESS, HDC1000_SERIALIDMID_REGISTER, 2)
        serialNumber = serialNumber*256 + data[0]*256 + data[1]

        data = self._bus.read_i2c_block_data(
            HDC1000_ADDRESS, HDC1000_SERIALIDBOTTOM_REGISTER, 2)
        serialNumber = serialNumber*256 + data[0]*256 + data[1]
