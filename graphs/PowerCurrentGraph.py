...
def PowerCurrentGraph(source, days, delay):
    print(f"PowerCurrentGraph source: {source} days: {days} delay: {delay}")
    ...
    try:
        ...
        query = (
            "SELECT TimeStamp, solarCurrent, batteryCurrent, loadCurrent "
            "FROM PowerSystem "
        ...
        pylab.figtext(.5, .05, f"GroveWeatherPi Power Current Last {days} Days", fontsize=18, ha='center')
        ...
        del t, s, u, v
        gc.collect()
        print("PowerCurrentGraph finished now")
