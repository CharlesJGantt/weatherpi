...
def TemperatureHumidityGraph(source, days, delay):
    print(f"TemperatureHumidityGraph source:{source} days:{days}")
    print(f"sleeping seconds: {delay}")
    time.sleep(delay)
    ...
    try:
        ...
        query = (
            "SELECT TimeStamp, bmp180Temperature, outsideTemperature, outsideHumidity, insideHumidity "
            "FROM WeatherData "
        ...
        pylab.figtext(.5, .05, f"Environmental Statistics Last {days} Days", fontsize=18, ha='center')
