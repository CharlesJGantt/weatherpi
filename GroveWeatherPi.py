...
command_file = os.path.join(home_dir, "SDL_Pi_GroveWeatherPi", "state", "WeatherCommand.txt")
import math
import re
from datetime import datetime
import time
import sys
sys.path.append('/home/benchpi/SDL_Pi_GroveWeatherPi/RTC_SDL_DS3231')
sys.path.append('/home/benchpi/SDL_Pi_GroveWeatherPi/graphs')
...
from SDL_Pi_FRAM.SDL_Pi_FRAM import SDL_Pi_FRAM
from SDL_Pi_WeatherRack.SDL_Pi_WeatherRack import SDL_Pi_WeatherRack
from RTC_SDL_DS3231.SDL_DS3231 import SDL_DS3231
...
if (config.TCA9545_I2CMux_Present):
    tca9545.write_control_register(TCA9545_CONFIG_BUS0)
weatherStation = SDL_Pi_WeatherRack(
...
def process_as3935_interrupt():
    ...
    if reason == 0x00:
        ...
        sendemail.sendEmail(
            "test",
            "GroveWeatherPi Lightning Detected\n",
            as3935LastStatus,
            config.textnotifyAddress,
            config.fromAddress,
            "")
...
def returnPercentLeftInBattery(currentVoltage, maxVolt):
    ...
    if (scaledVolts > 0.9374):
        returnPercent = 10 * \
            (1 - (0.9686 - scaledVolts) / (0.9686 - 0.9374)) + 80
        return returnPercent

    if (scaledVolts > 0.9063):
        returnPercent = 30 * \
            (1 - (0.9374 - scaledVolts) / (0.9374 - 0.9063)) + 50
        return returnPercent

    if (scaledVolts > 0.8749):
        returnPercent = 20 * \
            (1 - (0.8749 - scaledVolts) / (0.9063 - 0.8749)) + 11

        return returnPercent

    if (scaledVolts > 0.8437):
        returnPercent = 15 * \
            (1 - (0.8437 - scaledVolts) / (0.8749 - 0.8437)) + 1
        return returnPercent

    if (scaledVolts > 0.8126):
        returnPercent = 7 * (1 - (0.8126 - scaledVolts) /
                             (0.8437 - 0.8126)) + 2
        return returnPercent

    if (scaledVolts > 0.7812):
        returnPercent = 4 * (1 - (0.7812 - scaledVolts) /
                             (0.8126 - 0.7812)) + 1
        return returnPercent
...
def readWXLink(block1, block2):
    ...
    if ((len(block1) > 0) and (len(block2) > 0)):
        ...
        if (receivedCRC == calculatedCRC):
            ...
            WXbatteryCharge = returnPercentLeftInBattery(
...
def sampleWeather():
    ...
    if (config.BMP280_Present):
        try:
            ...
            bmp180SeaLevel = bmp280.read_sealevel_pressure(
                config.BMP280_Altitude_Meters) / 1000
        except BaseException:
            print(("Unexpected error:", sys.exc_info()[0]))
    ...
    if (config.HTU21DF_Present):
        # We use a C library for this device as it just doesn't play well with
        # Python and smbus/I2C libraries
        HTU21DFOut = subprocess.check_output(["htu21dflib/htu21dflib", "-l"])
    ...
    if (config.Sunlight_Present):
        ...
        SunlightVisible = SI1145Lux.SI1145_VIS_to_Lux(
            Sunlight_Sensor.readVisible())
    ...
    if (config.WeatherUnderground_Present):
        ...
        try:
            ...
            WeatherUnderground.sendWeatherUndergroundData(
                as3935LightningCount,
                as3935,
                as3935LastInterrupt,
                as3935LastDistance,
                as3935LastStatus,
...
def sampleSunAirPlus():
    ...
    if (config.SunAirPlus_Present):
        ...
        # minus is to get the "sense" right.   - means the battery is charging,
    ...
    try:
        ...
        if (config.OLED_Present):
            Scroll_SSD1306.addLineOLED(
                display, ("Wind Speed=\t%0.2f MPH") %
                (currentWindSpeed / 1.6))
            Scroll_SSD1306.addLineOLED(
                display, ("Rain Total=\t%0.2f in") %
                (totalRain / 25.4))
            if (config.ADS1015_Present or config.ADS1115_Present):
                Scroll_SSD1306.addLineOLED(
                    display, "Wind Dir=%0.2f Degrees" %
...
def writeWeatherRecord():
    ...
    try:
        ...
        query = 'INSERT INTO WeatherData(TimeStamp,as3935LightningCount, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain,  bmp180Temperature, bmp180Pressure, bmp180Altitude,  bmp180SeaLevel,  outsideTemperature, outsideHumidity, currentWindDirection, currentWindDirectionVoltage, insideTemperature, insideHumidity) VALUES(UTC_TIMESTAMP(), %.3f, %.3f, %.3f, "%s", %.3f, %.3f, %.3f, %i, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)' % (
            as3935LightningCount, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain, bmp180Temperature, bmp180Pressure, bmp180Altitude, bmp180SeaLevel, outsideTemperature, outsideHumidity, currentWindDirection, currentWindDirectionVoltage, HTUtemperature, HTUhumidity)
        print(("query=%s" % query))
        ...
        if (config.Sunlight_Present):
            ...
            query = 'INSERT INTO Sunlight(TimeStamp, Visible, IR, UV, UVIndex) VALUES(UTC_TIMESTAMP(), %d, %d, %d, %.3f)' % (
                SunlightVisible, SunlightIR, SunlightUV, SunlightUVIndex)
            print(("query=%s" % query))
            cur.execute(query)
...
def writePowerRecord():
    ...
    try:
        ...
        query = 'INSERT INTO PowerSystem(TimeStamp, batteryVoltage, batteryCurrent, solarVoltage, solarCurrent, loadVoltage, loadCurrent, batteryPower, solarPower, loadPower, batteryCharge) VALUES (UTC_TIMESTAMP (), %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)' % (
            batteryVoltage, batteryCurrent, solarVoltage, solarCurrent, loadVoltage, loadCurrent, batteryPower, solarPower, loadPower, batteryCharge)
...
def shutdownPi(why):
    ...
    sendemail.sendEmail(
        "test",
        "GroveWeatherPi Shutting down:" + why,
        "The GroveWeatherPi Raspberry Pi shutting down.",
...
def WLAN_check():
    ...
    if (config.enable_WLAN_Detection):
        ping_ret = subprocess.call(
            [
                'ping -c 2 -w 1 -q ' +
                config.PingableRouterAddress +
                ' |grep "1 received" > /dev/null 2> /dev/null'],
            shell=True)

        print(
            "checking WLAN:  ping_ret=%i WLAN_check_flg=%i" %
            (ping_ret, WLAN_check_flg))
        if ping_ret:
            # we lost the WLAN connection.
            # did we try a recovery already?
            if (WLAN_check_flg > 2):
                # we have a serious problem and need to reboot the Pi to
                # recover the WLAN connection
...
def statusRain():
    if (config.USEBLYNK):
        updateBlynk.blynkStatusTerminalUpdate(
            "Rain in past 60 minutes=%0.2fmm" %
            rain60Minutes)
...
def checkForButtons():
    ...
    if ((state.runOLED == False) and (config.OLED_Originally_Present)):
        reinitializeOLED = True

    updateBlynk.blynkStatusUpdate()

    if ((state.runOLED) and (reinitializeOLED)):
        I2C_Lock.acquire()
        initializeOLED()
        I2C_Lock.release()
...
print("")
print(
    "GroveWeatherPi Solar Powered Weather Station Version " +
    GWPVERSION +
    " - SwitchDoc Labs")
print("")
print("")
...
pclogging.log(
    pclogging.INFO,
    __name__,
    "GroveWeatherPi Startup Version" +
    GWPVERSION)
...
bodyText = "GroveWeatherPi Version " + \
    GWPVERSION + " Startup \n" + ipAddress + "\n"
if (config.SunAirPlus_Present):
    sampleSunAirPlus()
...
scheduler.add_job(patTheDog, 'interval', seconds=10)

# every minute, check for button changes
scheduler.add_job(
    checkForButtons,
    'interval',
    seconds=60)   # reset the WatchDog Timer
