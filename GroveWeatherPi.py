...
def sampleWeather():
    ...
    if (config.WeatherUnderground_Present == True):
        ...
        try:
            ...
            WeatherUnderground.sendWeatherUndergroundData(as3935LightningCount, as3935, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain, bmp180Temperature,
                                                          bmp180SeaLevel, bmp180Altitude,  bmp180SeaLevel, outsideTemperature, outsideHumidity, crc_check, currentWindDirection, currentWindDirectionVoltage, HTUtemperature, HTUhumidity, rain60Minutes)
        except:
    ...
    try:
        ...
        if (config.OLED_Present):
            Scroll_SSD1306.addLineOLED(
                display,  ("Wind Speed=\t%0.2f MPH") % (currentWindSpeed/1.6))
            Scroll_SSD1306.addLineOLED(
                display,  ("Rain Total=\t%0.2f in") % (totalRain/25.4))
        ...
        if (config.DS3231_Present == True):
            ...
            if (config.OLED_Present):
                Scroll_SSD1306.addLineOLED(
                    display, "%s" % ds3231.read_datetime())
        ...
        if (config.HDC1080_Present):
            if (config.OLED_Present):
                Scroll_SSD1306.addLineOLED(
                    display,  "InTemp = \t%0.2f C" % HTUtemperature)
...
def writeWeatherRecord():
    ...
    try:
        ...
        con = mdb.connect('localhost', 'root',
                          config.MySQL_Password, 'GroveWeatherPi')
...
def shutdownPi(why):
    ...
    sendemail.sendEmail("test", "GroveWeatherPi Shutting down:" + why,
                        "The GroveWeatherPi Raspberry Pi shutting down.", config.notifyAddress,  config.fromAddress, "")
    sys.stdout.flush()
    time.sleep(10.0)
...
def WLAN_check():
    ...
    if (config.enable_WLAN_Detection == True):
        ping_ret = subprocess.call(['ping -c 2 -w 1 -q '+config.PingableRouterAddress +
                                   ' |grep "1 received" > /dev/null 2> /dev/null'], shell=True)
...
def statusRain():
    if (config.USEBLYNK):
        updateBlynk.blynkStatusTerminalUpdate(
            "Rain in past 60 minutes=%0.2fmm" % rain60Minutes)
...
print("")
print("GroveWeatherPi Solar Powered Weather Station Version " +
      GWPVERSION+" - SwitchDoc Labs")
print("")
print("")
...
pclogging.log(pclogging.INFO, __name__,
              "GroveWeatherPi Startup Version"+GWPVERSION)
...
# reset the WatchDog Timer
scheduler.add_job(checkForButtons, 'interval', seconds=60)
...
scheduler.add_job(rebootPi, 'cron', day='2-30/2', hour=0,
                  minute=4, args=["48 Hour Reboot"])
