...
class SDL_Pi_WeatherRack:
    ...
    def current_wind_direction(self):
        ...
        direction = voltageToDegrees(
            voltageValue, SDL_Pi_WeatherRack._currentWindDirection)
        return direction
    ...
    def get_current_wind_speed_when_sampling(self):
        ...
        if (micros() - SDL_Pi_WeatherRack._startSampleTime >= compareValue):
            ...
            SDL_Pi_WeatherRack._currentWindSpeed = (float(
                SDL_Pi_WeatherRack._currentWindCount)/float(timeSpan)) * WIND_FACTOR*1000000.0
    ...
    def get_current_rain_total(self):
        rain_amount = 0.2794 * \
            float(SDL_Pi_WeatherRack._currentRainCount)/SDL_RAIN_BUCKET_CLICKS
        SDL_Pi_WeatherRack._currentRainCount = 0
        return rain_amount
    ...
    def current_wind_speed(self):  # in milliseconds
        ...
        if (SDL_Pi_WeatherRack._selectedMode == SDL_MODE_SAMPLE):
            ...
            SDL_Pi_WeatherRack._currentWindSpeed = (float(
                SDL_Pi_WeatherRack._currentWindCount)/float(SDL_Pi_WeatherRack._sampleTime)) * WIND_FACTOR
