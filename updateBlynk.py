...
def blynkInit():
    ...
    try:
        ...
        try:
            f = open(
                "/home/pi/SDL_Pi_GroveWeatherPi/state/EnglishMetric.txt", "r")
            value = int(f.read())
            f.close()
...
def blynkStateUpdate():
    ...
    try:
        ...
        # wind direction
        val = "{0:0.0f}/".format(state.ScurrentWindDirection) + \
            util.returnWindDirection(state.ScurrentWindDirection)
        put_body = json.dumps([val])
        r = requests.put(
            config.BLYNK_URL +
            config.BLYNK_AUTH +
    ...
    try:
        ...
        if (r.text == '["1"]'):
            if (state.EnglishMetric == 0):
                ...
                f = open(
                    "/home/pi/SDL_Pi_GroveWeatherPi/state/EnglishMetric.txt", "w")
                f.write("1")
                f.close()
        else:
            ...
            if (state.EnglishMetric == 1):
                ...
                if (DEBUGBLYNK):
                    print(
                        "blynkStatusUpdate:POSTBRC:state.EnglishMetric set to English")
        ...
        if (r.text == '["1"]'):
            if (state.runOLED == False):
                ...
                if (config.OLED_Originally_Present):
