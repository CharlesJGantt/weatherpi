...
def addLineOLED(display, text):
    ...
    draw.rectangle((0, lineheight * currentLine, 127,
                   lineheight * (currentLine + 1) - 1), outline=0, fill=0)
    ...
    if (currentLine == 5):
        for i in range(0, 5):
            j = 5 - i - 1
            draw.rectangle((0, lineheight * (j), 127,
                           lineheight * (j + 1) - 1), outline=0, fill=0)
