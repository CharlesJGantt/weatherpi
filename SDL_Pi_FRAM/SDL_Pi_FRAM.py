...
class SDL_Pi_FRAM():
    ...
    def write8(self, address, data):
        # print "addr =0x%x address = 0x%x data = 0x%x  " % (self._addr,
        # address, data)
    self._bus.write_i2c_block_data(
        self._addr, address >> 8, [
            address %
            256, data])
    ...
    # this will read at the current address pointer, which we on the previous
    # line
    returndata = self._bus.read_byte(self._addr)
    # print "addr = 0x%x address = 0x%x %i returndata = 0x%x " % (self._addr,
    # address, address, returndata)
    return returndata
